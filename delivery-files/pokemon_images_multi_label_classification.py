...
def execute_model_of_multi_label_classification_for_all_available_optimisers(final_choice=None):
    ...
    for num_optimiser in range(NUM_AVAILABLE_OPTIMISERS):
        ...
        if (AVAILABLE_OPTIMISERS_LIST[num_optimiser] == final_choice) and (final_choice is not None):
            ...
            # the Model for the feed-forward Convolution Neural Network (C.N.N.)
            cnn_model_in_keras_sequential_api_for_image_classification_multi_labels_classification \
                .save_weights('pokemon-image-classification-training-history-multi-labels-%s-optimiser-%s-weights.h5'
            ...
            with open('pokemon-image-classification-training-history-multi-labels-%s-optimiser-%s-weights.json'
